...
export async function POST() {
  try {
    ...
    // Try to create table - if it fails, table already exists
    const createOrgResponse = await fetch(`${supabaseUrl}/rest/v1/rpc/exec_sql`, {
      method: 'POST',
      headers: {
    ...
    if (!createOrgResponse.ok && !orgError) {
      const error = await createOrgResponse.json()
      console.error('Organizations error:', error)
      return NextResponse.json({ error: 'Failed at organizations', details: error }, { status: 500 })
    }

    console.log('✅ Organizations table created')

    // Migration 2: VoIP Users table
    const users_sql = `
    ...
    const { error: usersError } = await supabase.rpc('exec_sql', { sql: users_sql })

    if (usersError) {
      console.error('VoIP Users error:', usersError)
      return NextResponse.json({ error: 'Failed at voip_users', details: usersError }, { status: 500 })
    }

    console.log('✅ VoIP Users table created')

    // Migration 3: Calls table
    const calls_sql = `
    ...
    const { error: callsError } = await supabase.rpc('exec_sql', { sql: calls_sql })

    if (callsError) {
      console.error('Calls error:', callsError)
      return NextResponse.json({ error: 'Failed at calls', details: callsError }, { status: 500 })
    }
